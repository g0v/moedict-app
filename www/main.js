...
(function(){
  var isCordova, isMoedictDesktop, DEBUGGING, ref$, STANDALONE, ref1$, any, map, React, Han, LANG, MOEID, XREFLABELOF, TITLEOF, HASHOF, STARRED, key, LRU, res$, isQuery, isDroidGap, isDeviceReady, isMobile, isApp, isWebKit, isGecko, isChrome, isPrerendered, widthIsXs, entryHistory, INDEX, XREF, CACHED, addToLru, GET, e, playing, player, seq, getEl, callLater, MOE, PUA2UNI, trs_lookup, pinyin_lookup, han_amis_lookup, SIMPTRAD, httpMap, LoadedScripts, split$ = ''.split, replace$ = ''.replace, join$ = [].join;
  ...
  function initAutocomplete(){
    ...
    return $('#query').autocomplete({
      ...
      source: function(arg$, cb){
        ...
        if (LANG === 'a' && /^[a-zA-Z1-4 ]+$/.exec(term)) {
          return pinyin_lookup(term, cb);
        }
  ...
  han_amis_lookup = function(query, cb){
    GET('revdict-amis-def.txt', function(cmn_amis_def){
      return GET('revdict-amis-ex.txt', function(cmn_amis_ex){
        var x, terms, lookup_in, this$ = this;
        x = [];
